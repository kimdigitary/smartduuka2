...
class SendSubscriptionReminders extends Command
{
    ...
    private function processReminder(Tenant $tenant): void
    {
        ...
        if ($template && $subject) {
            SendEmailsJob::dispatch(
                $this->subscriptionReminderRecipients($onboard->admin_email),
                $subject,
                $template,
                $emailData
            );
     ...
     */
    private function subscriptionReminderRecipients(?string $onboardAdminEmail): array
    {
        return User::role(Role::ADMIN)
            ->whereNotNull('email')
            ->pluck('email')
            ->prepend($onboardAdminEmail)
            ->filter()
            ->unique()
            ->values()
            ->all();
