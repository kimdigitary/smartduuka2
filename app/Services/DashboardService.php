...
class DashboardService
{
    ...
    public function quotations(Request $request): array
    {
        try {
            ...
            $currentTotal = $this->quotationQuery($startDate, $endDate)->count();
            $previousTotal = $this->quotationQuery($prevStartDate, $prevEndDate)->count();
            $currentConverted = $this->quotationQuery($startDate, $endDate)
                ->where('quotation_status', QuotationStatus::CONVERTED)
                ->count();
            $previousConverted = $this->quotationQuery($prevStartDate, $prevEndDate)
                ->where('quotation_status', QuotationStatus::CONVERTED)
                ->count();

            $conversionRate = $this->percentageOf($currentConverted, $currentTotal);
            $previousConversionRate = $this->percentageOf($previousConverted, $previousTotal);

            return [
                'total' => [
                    'value' => $currentTotal,
                    'change' => $this->percentageChange($currentTotal, $previousTotal),
                ],
                'converted' => [
                    'value' => $currentConverted,
                    'change' => $this->percentageChange($currentConverted, $previousConverted),
                ],
                'conversion_rate' => [
                    'value' => $conversionRate,
                    'change' => round($conversionRate - $previousConversionRate, 1),
                ],
            ];
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
    ...
    public function kpi(Request $request)
    {
        try {
            ...
            $expenses = Expense::whereBetween('date', [$startDate, $endDate])
                ->where('expense_nature', ExpenseNature::OPERATIONAL)
                ->sum('amount');
    ...
    private function quotationQuery(Carbon $startDate, Carbon $endDate)
    {
        return Order::query()
            ->whereBetween('order_datetime', [$startDate, $endDate])
            ->where(function ($query) {
                $query->where('payment_type', PaymentType::QUOTATION)
                    ->orWhere('order_type', OrderType::QUOTATION)
                    ->orWhere('order_type', PaymentMethodEnum::QUOTATION)
                    ->orWhere('original_type', PaymentMethodEnum::QUOTATION);
            });
    }
    ...
    private function percentageOf(int|float $value, int|float $total): float
    {
